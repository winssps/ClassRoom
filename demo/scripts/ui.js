...
/*
FileProgress.prototype.sendMessageToServer = function(file,up,info) {
    ...
    var res = $.parseJSON(info);
        ...
        var link = domain + res.key;
        console.log(file);
        var data = {"filename":file.name,"filesize":file.size,"link":link,"url":url};
    $.ajax({//  向服务器post请求
         url:'/fileMessage',
         type:'POST',
         data:data,
         success:function(backdata,status){//请求成功
            if(status == 'success'){//发送完之后，更新列表
             // var userserverfile = new userFileList(backdata);
             }
          },
